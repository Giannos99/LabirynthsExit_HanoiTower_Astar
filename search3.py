...
class MapProblem(Problem):
     ...
     def __init__(self, initial, goal, mymap,n):#n =diastash tou map
         Problem.__init__(self, initial, goal)
         self.map = mymap
         self.dimension=n
         self.numberofexpandednodes=0
     ...
     def actions(self, state):
        ...
        iterator, rather than building them all at once."""
        listofactions=[]
         #self.map[state[0]][state[1]]=3
        up=(-1,0)
        down=(1,0)
        left=(0,-1)
        right=(0,1)
        if state[1]!=0 :
            if (self.map[state[0]][state[1]-1])!=1:#elegxei an exei empodio pros ta aristera
                listofactions.append(left)
        if state[1]!=self.dimension-1 :
            if (self.map[state[0]][state[1]+1])!=1:
                listofactions.append(right)
        if state[0]!=0 :
            if (self.map[state[0]-1][state[1]])!=1:
                listofactions.append(up)
        if state[0]!=self.dimension-1:
            if (self.map[state[0]+1][state[1]])!=1:
     ...
     def result(self, state, action):   
         ...
         return nextstate
     def path_cost(self, c, state1, action, state2):
        if (self.map[state2[0]][state2[1]]==2):
            return c+2
        return c + 1
     def h1(self,n):
         return 1
     ...
     def h2(self,n):
         squareroot= ((self.goal[0]-n.state[0])*(self.goal[0]-n.state[0])+\
                      (self.goal[1]-n.state[1])*(self.goal[1]-n.state[1]))**1/2
         return squareroot
         
     def h3(self,n):#manhattan
         manhattan = abs(self.goal[0] - n.state[0]) + abs(self.goal[1] - n.state[1])
         return manhattan
...
class Hanoi(Problem):
    ...
    def __init__(self, initial, goal, mymap,n):#n =diastash tou map
         Problem.__init__(self, initial, goal)
         self.map = mymap#auth th fora to mymap..einai tuple me  3 tuples
         self.dimension=n
         self.numberofexpandednodes=0
    def actions(self, state):
        listofactions=[]
        ...
        if ((len(state[0]))!=0) and ((len(state[1]))!=0):#an kamia ap tis duo den einai adeia
            if state[0][len(state[0])-1]>state[1][len(state[1])-1]:#ama to teleutaio stoixeio ths prwths listas mporei na mpei sthn deuterh
                listofactions.append(a1)
            else:
                listofactions.append(a3)
        if ((len(state[0]))!=0) and ((len(state[2]))!=0):
            if state[0][len(state[0])-1]>state[2][len(state[2])-1]:
                listofactions.append(a2)
            else:
                listofactions.append(a5)
        if ((len(state[1]))!=0) and ((len(state[2]))!=0):
            if state[1][len(state[1])-1]>state[2][len(state[2])-1]:
                listofactions.append(a4)
            else:
                listofactions.append(a6)
        if ((len(state[0]))==0):#h prwth sthlh einai kenh
            if (len(state[1])!=0):
                listofactions.append(a3)
            if (len(state[2])!=0):
                listofactions.append(a5)
    ...
    def path_cost(self, c, state1, action, state2):
        return c+1
    def result(self, state, action):
        list0, list1, list2 = list(state[0]), list(state[1]), list(state[2])
        if action=='1to2':#sthn apo panw grammh...apo8hkeuoume prosorina ta tuple san listes wste na ta metavaloume,kai na paroume to next state
            list1.append(list0[len(state[0])-1])#ap thn 1 sthn 2...vazoume ston 2o stulo
            list0.pop()#to teleutaio stoixeio tou 1ou stulou..kai meta to kanoume pop
        if action=='1to3':
            list2.append(list0[len(state[0])-1])
            list0.pop()
        if action=='2to1':
            list0.append(list1[len(state[1])-1])
            list1.pop()
        if action=='2to3':
            list2.append(list1[len(state[1])-1])
            list1.pop()
        if action=='3to1':
            list0.append(list2[len(state[2])-1])
            list2.pop()
        if action=='3to2':
            list1.append(list2[len(state[2])-1])
            list2.pop()
        return (tuple(list0), tuple(list1), tuple(list2))
    def h1(self,n):
        return (len(n.state[1])-1)
...
def breadth_first_search(problem):
    ...
    return None
#problem.numberofexpandednodes+=1
...
listofobtacles=[]
listofdoublecost=[]
f = open( 'input.txt' )#anoigma,by default gia anagnwsh mono


while 1:#edw mesa ftiaxnw me thn xrhsh 2 listwn...mia lista apo listes pou krata ena ena ta stoixeia ka8e grammhs tou plegmatos
    char = f.read(1)          # read by character
    if (char=='\n') or (not char):
        listOfLists.append(singleList)
        singleList=[]
    if not char:
        break
    if(char!='\n'): 
        singleList.append(char)
f.seek(0)# go back to byte 0 of the f i l e    
n =len( listOfLists)#autos einai o ari8mos twn grammwn k twn sthlwn sto nxn,provlhma mas

#print(n)
counter=0
goal=(0,0)
fstate=(0,0)
################### Apo edw 3ekinaei to Hanoi problem #########################################
hanoiNo=input("Plz give number of disks for Hanoi problem? ")
hanoi=(tuple(range(hanoiNo)),(),())
goalhanoi=((),(),tuple(range(hanoiNo)))

print 'hanoi_goal= ',goalhanoi
print 'hanoi_initial= ',hanoi
h=Hanoi(hanoi,goalhanoi,hanoi,hanoiNo)
#print(g.map[fstate[0]][fstate[1]])
s_hanoi = astar_search(h, h.h1)
sol = s_hanoi.solution() # The sequence of actions to go from the root to this node
path = s_hanoi.path() # The nodes that form the path from the root to this node
...
for i in range(len(path)) :
	...
	if i > 0 : # The initial state has not an action that results to it
		action = sol[i-1]
	print "|%s\t|%s \t|%s \t   |"%(action, state, cost)
print "+%s+"%('-'*42)
print('number of nodes expanded is', h.numberofexpandednodes)

############################### Mexri edw einai to Hanoi problem ##########################
for i in range(n):
    for j in range(n):
        counter+=1
        listOfLists [i ][j]=int(listOfLists [i ][j])#kanw ta char,tou pinaka se int
        #print(listOfLists [i ][j])
        if (listOfLists [i ][j]==4):
            goal=(i,j)
        if listOfLists [i][j]==3:
            fstate=(i,j)
        if listOfLists [i][j]==2:
            listofdoublecost.append((i,j))#lista me tis 8eseis twn doublecost
        if listOfLists [i][j]==1:
            listofobtacles.append((i,j))#lista me thn 8esh twn empodiwn
#print(listOfLists)
print '\n\nMAIZE PROBLEM \n\ngoal= ',goal
print'first state= ',fstate
g=MapProblem(fstate,goal,listOfLists,n)
#print(g.map[fstate[0]][fstate[1]])
s = astar_search(g, g.h3)
...
for i in range(len(path)) :
	...
	print "|%s\t|%s \t|%s \t   |"%(action, state, cost)
print "+%s+"%('-'*42)
print('number of nodes expanded is', g.numberofexpandednodes)
f.close()
